  ...
  return (
    <header className={`header ${loggedIn || minimal? 'header_theme_dark' : ''}`}>
      <div className={`header__container ${minimal ? 'header__container_small' : ''}`}>
        ...
        <div className={`header__account ${loggedIn || authorized ? 'header__account_visible' : ''}`}>
          <NavLink to='/profile' className='header__list-link' activeClassName='header__list-item_active'>
            Аккаунт
          </NavLink>
        </div>
        </div>
        <button className={`header__burger ${loggedIn || authorized ? 'header__burger_visible' : ''}`} onClick={openMenu}>
          <span className='header__burger-span'></span>
        </button>
        <div className={`header__buttons ${loggedIn || minimal || authorized ? 'header__buttons_hidden' : ''}`}>
          <Link to='/signup'>
